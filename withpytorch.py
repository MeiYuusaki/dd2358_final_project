...
def predict(Theta1, Theta2, X):
	...
	torch_Theta1 = torch.from_numpy(Theta1.T)
	torch_Theta2 = torch.from_numpy(Theta2.T)

	a1 = torch.hstack((torch.ones((m,1)), torch_X))   # add bias (input layer)
	a2 = torch_g(a1 @ torch_Theta1)                 # apply sigmoid: input layer --> hidden layer
	a2 = torch.hstack((torch.ones((m,1)), a2))  # add bias (hidden layer)
	a3 = torch_g(a2 @ torch_Theta2)                 # apply sigmoid: hidden layer --> output layer
	
	prediction = np.argmax(a3.numpy(),1).reshape((m,1))
	return prediction
...
def cost_function(theta, input_layer_size, hidden_layer_size, num_labels, X, y, lmbda):
	...
	a1 = torch.hstack((torch.ones((m,1)), torch_X))   
	a2 = torch_g(a1 @ torch_Theta1)                 
	a2 = torch.hstack((torch.ones((m,1)), a2))  
	a3 = torch_g(a2 @ torch_Theta2)
	...
	# cost function							)
	J = np.sum( -y_mtx * np.log(a3.numpy()) - (1.0-y_mtx) * np.log(1.0-a3.numpy()) ) / m
...
def gradient(theta, input_layer_size, hidden_layer_size, num_labels, X, y, lmbda):
	...
	# forward
	a1 = X.T
	a1 = np.vstack((np.ones((1,X.shape[0])), a1))  #  +bias
	z2 = Theta1 @ a1 
	a2 = g(z2)
	a2 = np.vstack((np.ones((1,X.shape[0])), a2))   #  +bias
	a3 = g(Theta2 @ a2)  
	
	# compute error for layer 3
	y_k = np.zeros((num_labels,X.shape[0]))   
	for t in range(m):
		y_k[y[t,0].astype(int),t] = 1 
...
def main():
	...
	# For 64x64
	#train = np.genfromtxt('train_scaled_64x64.csv', delimiter=',')
	#test = np.genfromtxt('test_scaled_64x64.csv', delimiter=',')

	# For 96x96
	#train = np.genfromtxt('train_scaled_96x96.csv', delimiter=',')
	#test = np.genfromtxt('test_scaled_96x96.csv', delimiter=',')
...
if __name__== "__main__":
	...
	for _ in range(20):
		...
		print(f"Program took {runtime} seconds.")
